...
cap1 = cv2.VideoCapture('workouts_files/new-squats-expert-fast-2.mp4')
recording = False
###-----------------------------------------------------------------------------------------FROM SPLITSCREEN
...
df_expert = pd.read_csv('workout_coor/new test data/new_squats_expert.csv')

rshoul_eX, rshoul_eY = getXY(df_expert['RAnkle']) 
relbow_eX, relbow_eY = getXY(df_expert['RKnee'])
rwrist_eX, rwrist_eY = getXY(df_expert['RHip'])

lshoul_eX, lshoul_eY = getXY(df_expert['LAnkle']) 
lelbow_eX, lelbow_eY = getXY(df_expert['LKnee'])
lwrist_eX, lwrist_eY = getXY(df_expert['LHip'])
        ...
        with open('new.csv', 'a') as csvfile: 
            ...
            try:
                ...
                if len(humans[0].body_parts.keys())-1 == 17:
                    ...
                    df_user = pd.read_csv('new.csv')

                    rshoul_uX, rshoul_uY = getXY(df_user['RAnkle']) 
                    relbow_uX, relbow_uY = getXY(df_user['RKnee'])
                    rwrist_uX, rwrist_uY = getXY(df_user['RHip'])

                    lshoul_uX, lshoul_uY = getXY(df_user['LAnkle']) 
                    lelbow_uX, lelbow_uY = getXY(df_user['LKnee'])
                    lwrist_uX, lwrist_uY = getXY(df_user['LHip'])
